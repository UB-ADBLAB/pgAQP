...
/* GUC variables for configuring AB-tree. */
...
extern bool abtree_read_agg_bypass_version_store;
extern bool abtree_lock_lrpath_during_sampling;
...
typedef struct ABTLockedPageData
{
    BlockNumber         alp_blkno; /* redundant */
    OffsetNumber        alp_off;
    Buffer              alp_buf;
} ABTLockedPageData, *ABTLockedPage;

typedef struct ABTLockedPathSubtreeRefDes {
    BlockNumber     blkno;
    Buffer          buf;
    OffsetNumber    minoff;
    OffsetNumber    maxoff;
    ...
    bool            in_use;
    bool            is_splited;
} ABTLockedPathSubtreeRefDes;

typedef struct ABTLockedPathSubtreeRef {
    uint16              idx;
    OffsetNumber        off;
} ABTLockedPathSubtreeRef;

typedef struct ABTLockedPathData
{
    /* Sub-tree root. If invalid, it indicates empty set to sample from. */
    BlockNumber         alp_root_blkno; 
    uint32              alp_level; /* sub-tree height */
    Buffer              alp_root_buf;
    OffsetNumber        alp_loff; /* left-most sub-tree's offset */
    OffsetNumber        alp_roff; /* right-most sub-tree's offset */

    SamplerRandomState  alp_randstate; /* for subtree sampling */

    /* 
     * agg cache (only when x-lock is used), or buffers for reading sub-tree
     * aggs into (when s-lock is used). This is separately palloc'd.
     */
    uint16                  alp_nsubtrees;
    uint16                  alp_nrangesubtrees;
    uint16                  alp_nleaf;
    uint64                  alp_total_aggs;
    uint64                  *alp_subtree_aggs;
    aqp_alias_u64           *alp_subtree_alias;
    ABTLockedPathSubtreeRef *alp_subtreerefs;

    ABTLockedPathSubtreeRefDes  *alp_subtreerefdess;
    /* 
     * A stack of page data for pages on the left-most and right-most paths
     * below the sub-tree root. This has the size of `alp_level * 2 + 1`.
     *
     * The page on the left-most path at `level` is stored at index `level * 2
     * + 1` (or parent level * 2 - 1).
     *
     * The page on the right-most path at `level` is stored at `level * 2` (or
     * parent level * 2 - 2).
     *
     * For convenience, the value of rootbuf is also stored in the buf field
     * at index `2 * alp_level`.
     */
    ABTLockedPageData   alp_lpd[FLEXIBLE_ARRAY_MEMBER];
} ABTLockedPathData, *ABTLockedPath;

typedef struct ABTFastSamplingContextData {
    Buffer          rootbuf;
    OffsetNumber    leafoff;
    bool            root;
    IndexTuple      expected_hikey;
    uint64          sampled_item_weight;
    OffsetNumber    minoff;
    OffsetNumber    maxoff;
} ABTFastSamplingContextData, *ABTFastSamplingContext;
...
typedef struct ABTSubtreeStatis
{
    uint32 idx;
    uint32 subtree_idx;
    double weight;
...
typedef struct ABTScanOpaqueData
{
    ...
    /* 
     * Information of the locked paths for the fast sampling optimization for
     * small range sampling.
     */
    ABTLockedPath   locked_path;
    ...
    uint32 curr_partitions;
    uint32 curr_level;
    uint64 initial_sample_size;
    bool   split_partition;
    uint32 sample_height;

    ABTSubtreeStatis *statis;
...
extern bool abtsamplebatchtuple(IndexScanDesc scan, double random_number); 
...
extern Buffer _abt_getstackbuf(Relation rel, ABTStack stack, BlockNumber child,
							   ABTCachedAggInfo agg_info);
extern OffsetNumber _abt_try_find_downlink(Page page, OffsetNumber start_off,
                                           BlockNumber cblkno);
...
extern void _abt_decrement_aggs_for_vacuum(Relation rel, Buffer leafbuf,
                                           Datum deleted_sum,
                                           ABTCachedAggInfo agg_info);
extern bool _abt_continue_descent(IndexScanDesc scan, int index,
...
extern double _abt_total_weight(IndexScanDesc scan);
#define _abt_sample(scan, random_number) \
    _abt_sample_extended((scan), (random_number), NULL)
extern bool _abt_sample_extended(IndexScanDesc scan, double random_number,
                                 ABTFastSamplingContext fsctx);
extern void _abt_build_start_inskey(IndexScanDesc scan, ScanDirection dir);
extern void _abt_build_lpath_batch(IndexScanDesc scan);
extern void _abt_build_lpath_for_fast_sampling(IndexScanDesc scan);
extern void _abt_build_lpath_for_fast_sampling_batch(IndexScanDesc scan);
extern bool _abt_fast_sampling_batch(IndexScanDesc scan, double random_number);
extern bool _abt_fast_sampling(IndexScanDesc scan, double random_number);
extern void _abt_release_lpath_for_fast_sampling(Relation rel,
                                                 ABTLockedPath so);
extern bool _abt_gettuple_from_leaf(IndexScanDesc scan);
