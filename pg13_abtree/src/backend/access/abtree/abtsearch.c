...
static void _abt_set_dummy_lpath(ABTScanOpaque so);
static ABTLockedPath _abt_create_lpath(Buffer buf, OffsetNumber loff,
                                       OffsetNumber roff);
static OffsetNumber _abt_find_downlink(Relation rel, Buffer *p_buf,
                                       OffsetNumber start_off,
                                       BlockNumber cblkno, bool can_moveright);

static void _abt_precompute_partition(IndexScanDesc scan);
static void _abt_precompute_leaf_partition(IndexScanDesc scan);


static void 
_abt_precompute_partition(IndexScanDesc scan)
{
	ABTScanOpaque so = (ABTScanOpaque) scan->opaque;
	ABTLockedPath lpath = so->locked_path;
	ABTLockedPathSubtreeRef *subtreeref;
	int leaf_num = lpath->alp_nleaf;

	/* check if plevel == 0 (rbuf == lbuf) */
	
	if (lpath->alp_level == 0)
	{
		so->statis = (ABTSubtreeStatis *)
			palloc0(sizeof(ABTSubtreeStatis) * 1);
		
		so->statis[0].idx = 0;
		so->statis[0].subtree_idx = 0;
		so->statis[0].sample_size = lpath->alp_total_aggs;
		so->total_partitions = 1;
	}
	else
	{
		so->statis = (ABTSubtreeStatis *)
			palloc0(sizeof(ABTSubtreeStatis) 
				* lpath->alp_nsubtrees);

		for (int subtree_idx = 0; subtree_idx < lpath->alp_nsubtrees; subtree_idx++)
		{
			subtreeref = &lpath->alp_subtreerefs[subtree_idx];

			if (subtreeref->idx < 2)
			{
				so->statis[0].idx = 0;
				so->statis[0].subtree_idx = 0;
				so->statis[0].sample_size += lpath->alp_subtree_aggs[subtree_idx];
			}
			else
			{
				int index = subtree_idx - leaf_num + 1;
				so->statis[index].idx = index;
				so->statis[index].subtree_idx = subtree_idx;
				so->statis[index].weight = (double)
					lpath->alp_subtree_aggs[subtree_idx] 
						/ (so->locked_path->alp_total_aggs - so->statis[0].sample_size);
			}
		}

		so->total_partitions = lpath->alp_nsubtrees - leaf_num + 1;
	}
}


static void 
_abt_precompute_leaf_partition(IndexScanDesc scan)
{
	ABTScanOpaque so = (ABTScanOpaque) scan->opaque;
	ABTLockedPath lpath = so->locked_path;
	uint32 leaf_num = lpath->alp_nleaf;
	...
	{
		for (int subtree_idx = 0; subtree_idx < leaf_num; subtree_idx++)
		{
			so->statis[0].sample_size += lpath->alp_subtree_aggs[subtree_idx];
		}
	}
...
bool
_abt_gettuple_from_leaf(IndexScanDesc scan)
{
	ABTScanOpaque so = (ABTScanOpaque) scan->opaque;
	ABTLockedPath lpath = so->locked_path;
	ABTLockedPathSubtreeRef *subtreeref;
	...
	OffsetNumber	chosen_leaf_item_offnum,
					chosen_leaf_item_posting_off;
	bool	continuescan;
    uint32	subtree_idx;

	subtree_idx = so->idx_leaf;

	subtreeref = &lpath->alp_subtreerefs[subtree_idx];

	buf = lpath->alp_lpd[subtreeref->idx].alp_buf;
	Assert(BufferIsValid(buf));

	page = BufferGetPage(buf);
	opaque = (ABTPageOpaque) PageGetSpecialPointer(page);

	chosen_leaf_item_offnum = subtreeref->off;

	chosen_leaf_item_posting_off = so->currPos.itemIndex;
	...
	if (so->currPos.itemIndex == lpath->alp_subtree_aggs[so->idx_leaf] - 1)
	{
		so->idx_leaf++;
		so->currPos.itemIndex = 0;
	}
	...
	/* so->inv_prob = lpath->alp_subtree_aggs[subtree_idx] * (float8) lpath->alp_total_aggs /
					lpath->alp_subtree_aggs[subtree_idx];*/ 

	so->inv_prob = so->statis[0].sample_size;
...
/*
 * _abt_sample_extended() -- Sample one item from the range specified in the
 * scan.
 *
 *
 * rootbuf == InvalidBuffer -> start from real root
 * rootbuf != InvalidBuffer -> start from the specified subtree root.
 *
 * - When rootbuf is not invalid, it must be pinned and s-locked upon entry.
 * - If it points to an internal page, it will be unlocked and unpinned upon
 * return.
 * - If it points to a leaf page, it will not be unlocked or unpinned upon
 * return.
 */
bool
_abt_sample_extended(IndexScanDesc scan, double random_number,
                     ABTFastSamplingContext fsctx)
{
	Relation		rel = scan->indexRelation;
	ABTScanOpaque	so = (ABTScanOpaque) scan->opaque;
    ...
    if (fsctx) 
    {
        ...
        if (fsctx->leafoff > 0)
        {
            ...
            /* 
             * Leaf page is locked, and one data entry has been sampled.
             */
            itemid = PageGetItemId(page, fsctx->leafoff);
            itup = (IndexTuple) PageGetItem(page, itemid);
            
            chosen_leaf_item_offnum = fsctx->leafoff;
            ...
            if (ABTreeTupleIsPosting(itup))
            {
                ...
                if (agg_info->leaf_has_agg)
                {
                    ...
                    cutoff_value =
                        (uint64) floor(fsctx->sampled_item_weight * random_number);
                    agg_ptr = ABTreePostingTupleGetAggPtr(itup, agg_info);
                    for (i = 0; i < n; ++i)
                    {
                        /* 
                         * Note: do not exclude the invisible item here, as we
                         * counted them in the sampled_item_weight for posting
                         * tuples during the lpath locking procedure.
                         */
                        w = _abt_atomic_read_agg_ptr(agg_ptr, agg_info);
                        if (cutoff_value < w)
                        {
                            break;
                        } else {
                    ...
                    if (i == n)
                    {
                        chosen_leaf_item_posting_off = n - 1;
                    }
                    else
                    {
                ...
                /* 
                 * However, if we do find the tuple is invisible to us, we must
                 * report so to _abt_fast_sampling by preserving `so->inv_prob`.
                 */
                xid_ptr = ABTreePostingTupleGetXminPtr(itup);
                xid_ptr += chosen_leaf_item_posting_off;
                ...
                if (_abt_xid_in_snapshot(xid, snapshot))
                {
                    if (agg_info->leaf_has_agg)
                    {
                        so->inv_prob = 1.0 * fsctx->sampled_item_weight / w;
                    }
                    else
                    {
                        so->inv_prob = 1.0 * fsctx->sampled_item_weight;
                    }
                    return false;
                }
            }
            ...
            /* 
             * We have checked the xid for a regular leaf tuple, so we don't
             * need to check xid again if this is not a posting tuple.
             */
            
            /* 
             * done_sampling expects us to set so->inv_prob to the
             * non-pivot/posting tuple weight
             */
            so->inv_prob = fsctx->sampled_item_weight;
            goto done_sampling;
        }
        else
        {
			if (so->split_partition)
			{
				page = BufferGetPage(buf);
				opaque = (ABTPageOpaque) PageGetSpecialPointer(page);
				...
				minoff = fsctx->minoff; 
				maxoff = fsctx->maxoff;
				goto descend_with_minmax;
			}
			else
			{
				/*
				 * A subtree root is locked and we need to perform full tree
				 * sampling from here.
				 */
				page = BufferGetPage(buf);
				opaque = (ABTPageOpaque) PageGetSpecialPointer(page);
				...
				/*
				 * Check if this page is concurrently split.
				 */
				/* 
				 * XXX concurrent split cannot happen right now because 
				 * we'd had a deadlock here. FIXME if we allow concurrent split
				...
				/* 
				 * Also need to set up cutoff and acc values.
				 *
				 * XXX: strictly speaking, using sampled_item_weight is not
				 * concurrency-safe here, because one could concurrently insert an
				 * item in this subtree, and thus increment the actual weight in
				 * the parent node. However, the version store is supposed to
				 * closely track this number so that (1) the newly inserted tuple
				 * weights are not counted towards the accumulated weight, and thus
				 * there is only a very brief period of time when we may miss a few
				 * tuples due to concurrency. This needs to be fixed later but not
				 * needed for running aqp read-only queries separately from
				 * updates.
				 */
				acc = 0;
				cutoff = (uint64) floor(fsctx->sampled_item_weight * random_number);
				if (cutoff == fsctx->sampled_item_weight)
					--cutoff;
	...
	if (!so->qual_ok)
    {
        so->inv_prob = get_float8_infinity();
		return false;
    }
...
descend_with_minmax:
	...
	if (minoff == maxoff)
	{
		...
		if (ABT_P_ISLEAF(opaque))
		{
			...
			if (ABTreeTupleIsPosting(itup))
			{
				...
				if (!agg_info->leaf_has_agg)
				{
                    ...
                    /*
                     * We simply reject any invisible tuples if there is only
                     * one single entry matched in the entire tree, as we never
                     * descended down the tree using any sub-tree weights
                     * before this -- that means we never excluded them from
                     * the sampling space prior to this point.
                     */
                    xid_ptr = ABTreePostingTupleGetXminPtr(itup);
                    xid = (TransactionId) pg_atomic_read_u32(
                        (pg_atomic_uint32 *) &xid_ptr[cutoff_index]);
                    if (_abt_xid_in_snapshot(xid, snapshot))
                    {
                        /* rejected sample */
                        _abt_relbuf(rel, buf);
			...
			else
			{
                ...
                {
                    /*
                     * XXX: this is a fairly rare case where we only have
                ...
                /*
                 * See above in the posting tuple branch. Similar argument
                 * there.
                 */
		...
		else
		{
			...
			if (buf == InvalidBuffer)
			{
				...
				_abt_relbuf(rel, buf);
                /*XXX this is probably an error case */
                so->inv_prob = get_float8_infinity();
				return false;
			}
			...
			if (ABT_P_RIGHTMOST(opaque))
			{
                 ...
                 *
                 * NOTE(5/30/24): same as below above continue_descent.
                 */
	...
	for (;;) {
		...
		if (ABT_P_RIGHTMOST(opaque))
		{
             ...
             *
             *
             * NOTE(5/30/24): changed else branch to -1 from 1, as the
             * rmost high key should be larger than anything non-null, and
             * _abt_compare returns the comparison with the scan key
             * (expected_hikey) on lhs and the actual high key on the rhs.
             *
             * XXX: can -1 happen? Maybe not as B-link tree does not do
             * merges.
             */ 
			prev_cmpres = (expected_hikey == NULL) ? 0 : -1;
		}
...
done_sampling:
    ...
    if (agg_info->leaf_has_agg)
    {
        ...
        {
            agg_ptr = ABTreePostingTupleGetAggPtr(itup, agg_info);
            agg_ptr += agg_info->agg_stride * chosen_leaf_item_posting_off;
        }
        else
        {
            agg_ptr = ABTreeNonPivotTupleGetAggPtr(itup, agg_info);
						...
						&continuescan))
	{
		 ...
		 * rejected. This is the same as in _abt_readpage().
		 */
        if (!fsctx || fsctx->leafoff == 0)
        {
            /* 
             * Must not release the leaf page buffer that was locked for fast
             * sampling.
             */
		    _abt_relbuf(rel, buf);
        }
    ...
    {
        /* 
         * Pretend we have released the buffer so next call to
         * ABTScanPosUnpinIfUnpinned won't unpin this buffer.
         */
        so->currPos.buf = InvalidBuffer;
    }
...
double
_abt_total_weight(IndexScanDesc scan)
{
	Relation		rel = scan->indexRelation;
	ABTScanOpaque	so = (ABTScanOpaque) scan->opaque;
	ABTSnapshot		snapshot = so->snapshot;
    ...
    /* 
     * Different from the _abt_sample() code, we need to retry if a concurrent
     * split happened and we find a child page we descend into has a high key
     * that does not match the seperator key in the parent page. The main
     * reason is in this case, the total weight from the parent will be
     * slightly off anyway and it might be worthwhile to get the total weight
     * after all the updates are finished.
     *
     * XXX Note that we're assuming _abt_total_weight is only called when there
     * are very occasional insertion/deletion happening in the system.
     * Otherwise, the total weight/acceptance rate/rejection rate will vary
     * as new insertion/deletion continuously comes in, and our estimation of
     * the needed samples won't be accurate anyway.
     *
     * At some point, if we have ongoing sampling queries along with concurrent
     * updates, we might want to consider maintain a small ingestion buffer to
     * hold off updating the tree until at some "rendezvous" point (e.g., in
     * between a sampling query, or in between two phases of the
     * re-optimization). Essentially, we want to build a Bentley-Saxes based
     * sampling index similar to (SIGMOD '24) Practial Dynamic Extension to
     * Sampling Indexes, but with a different policy for performing incremental
     * merging of a lower-level run into a higher-level run.
     */
retry:
	/*
	 * Get the root or the fast root. Note that, a stale ``fastroot'' that is
...
quick_descend_path:
	...
	if (minoff == maxoff)
	{
		...
		if (ABT_P_ISLEAF(opaque))
		{
			...
			if (ABTreeTupleIsPosting(itup))
			{
				...
				if (!agg_info->leaf_has_agg)
				{
					 ...
					 * list. Choose the one at the cutoff value.
					 */
					tot_weight = ABTreeTupleGetNPosting(itup);
                    goto found_tot_weight;
				}
				/* 
                 * Otherwise, we still need to fall through to find the total
                 * weight.
				 */
			}
			else
			{
                ...
                if (agg_info->leaf_has_agg)
                {
                    ...
                    Pointer agg_ptr = ABTreeNonPivotTupleGetAggPtr(itup,
                                                                   agg_info);
                    tot_weight = _abt_atomic_read_agg_ptr(agg_ptr, agg_info);
                    goto found_tot_weight;
                }
                else
                {
		...
		{
            /* 
             * We don't bother to read the aggregate value here, as
             * _abt_sample() would use the sum of weights at a lower level
             * if one internal level only has one single matching entry, which
             * will be a tigher bound of the total weights.
             */

			/* 
             * Need to set up two things: the child page target blkno, and the
             * expected high key in the child page. See the comments below.
			 */
			blkno = ABTreeTupleGetDownLink(itup);
            ...
            // Otherwise, we found a concurrently split page. In most cases
            // this is the root, but it could also be something under the root
            // and we descended down here through the quick path (one entry matches
            // the search range). Don't bother fixing it by moving to the right
            // as we should expect someone to fix that at the root very soon (and
            // as a result, the total weight could also differ).
            _abt_relbuf(rel, buf);
			goto retry;
		}
	}
	...
	/* 
     * Compute the total weights of matching entries.
	 */
    tot_weight = 0;
	/* 
	 * Much of the loop is similar in both leaf and non-leaf cases, but we
	 * can save some cycles for determining whether a pivot tuple is posting or
	 * a non-pivot tuple is a pivot tuple when fetching the aggregation value
	 * pointers.
	 */
			...
			if (ABTreeTupleIsPosting(itup))
			{
				...
				if (agg_info->leaf_has_agg)
				{
					...
					for (i = 0; i < n; ++i)
					{
                        ...
                        tot_weight += _abt_atomic_read_agg_ptr(agg_ptr,
                                                               agg_info);
						agg_ptr += agg_info->agg_stride;
					}
				}
	...
	else
	{
		for (offnum = minoff; offnum <= maxoff; ++offnum)
		{
			itemid = PageGetItemId(page, offnum);
			itup = (IndexTuple) PageGetItem(page, itemid);
			tot_weight += _abt_read_agg(rel, itup, snapshot, agg_info);
		}
	}
...
bool
_abt_continue_descent(IndexScanDesc scan, int index,
                      AttrNumber partition_attno, bool dp)
{
	Relation		rel = scan->indexRelation;
	ABTScanOpaque	so = (ABTScanOpaque) scan->opaque;
	/* ScanKey			scankey = so->keyData; */
    ABTLockedPath 	lpath = so->locked_path;
    ABTLockedPathSubtreeRefDes *subtreerefdes;
	TupleDesc		itupdesc = RelationGetDescr(rel);
	Datum 			key_pre;
	Datum           key_new;
	int             par_i = 0;
    ...
    ItemId 			itemid;
    IndexTuple 		itup;
    OffsetNumber    cminoff,
                    cmaxoff,
					cmin;
	FmgrInfo   		*procinfo;

	Oid partition_collation;

	if (partition_attno <= 0 ||
		partition_attno > IndexRelationGetNumberOfKeyAttributes(rel))
		elog(ERROR, "invalid partition attribute %d for AB-tree descent",
			partition_attno);

	procinfo = index_getprocinfo(rel, partition_attno, ABTORDER_PROC);
	partition_collation = rel->rd_indcollation[partition_attno - 1];

	subtreerefdes = &lpath->alp_subtreerefdess[index - 1];

	minoff = subtreerefdes->minoff;
	maxoff = subtreerefdes->maxoff;

	/* procinfo = index_getprocinfo(rel, 1, ABTORDER_PROC); */

	if (minoff != maxoff)
	{
		bool isNull_p;

		if (subtreerefdes->buf != InvalidBuffer)
			page = BufferGetPage(subtreerefdes->buf);
		else
		{
			buf = _abt_getbuf(rel, subtreerefdes->blkno, ABT_READ);
			page = BufferGetPage(buf);
		}
		opaque = (ABTPageOpaque) PageGetSpecialPointer(page);
		itemid = PageGetItemId(page, minoff);
		...
		key_pre = index_getattr(itup, partition_attno, itupdesc, &isNull_p);
		cmin = minoff;
		...
		for (int i = minoff + 1; i <= maxoff; i++)
		{
			...
			key_new = index_getattr(itup, partition_attno, itupdesc, &isNull);

			result = DatumGetInt32(FunctionCall2Coll(procinfo,
													partition_collation,
													key_pre,
													key_new));
			...
			if (result != 0)
			{
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].buf = InvalidBuffer;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].blkno = subtreerefdes->blkno;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].minoff = cmin;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].maxoff = i - 1;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].is_leaf = false;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].in_use = true;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].is_splited = true;
		...
		if (par_i != 0)
		{
			...
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].blkno = subtreerefdes->blkno;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].minoff = cmin;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].maxoff = maxoff;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].is_leaf = false;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].in_use = true;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].is_splited = true;
			par_i++;
	...
	else
	{
		...
		{
			buf = _abt_getbuf(rel, subtreerefdes->blkno, ABT_READ);
			ppage = BufferGetPage(buf);
			pitemid = PageGetItemId(ppage, minoff);
			pitup = (IndexTuple) PageGetItem(ppage, pitemid);
			cblkno = ABTreeTupleGetDownLink(pitup);

			buf = _abt_relandgetbuf(rel, buf, cblkno, ABT_READ);
		}
		else
		{
			ppage = BufferGetPage(subtreerefdes->buf);
			pitemid = PageGetItemId(ppage, minoff);
			pitup = (IndexTuple) PageGetItem(ppage, pitemid);
			cblkno = ABTreeTupleGetDownLink(pitup);

			buf = _abt_getbuf(rel, cblkno, ABT_READ);
		}
		...
		/* internal page */
		cminoff = ABT_P_FIRSTDATAKEY(opaque) + 1;
		cmaxoff = PageGetMaxOffsetNumber(page);
		itemid = PageGetItemId(page, cminoff);
		itup = (IndexTuple) PageGetItem(page, itemid);
		
		key_pre = index_getattr(itup, partition_attno, itupdesc, &isNull_p);
		cmin = cminoff;

		for (off = cminoff + 1; off <= cmaxoff; ++off)
		{
			bool isNull;
			...
			{
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].buf = InvalidBuffer;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].blkno = cblkno;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].minoff = cmin;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].maxoff = off - 1;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].is_leaf = false;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].in_use = true;
				lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].is_splited = true;
		...
		if (par_i != 0)
		{
			...
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].blkno = cblkno;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].minoff = cmin;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].maxoff = cmaxoff;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].is_leaf = false;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].in_use = true;
			lpath->alp_subtreerefdess[so->total_partitions - 1 + par_i].is_splited = true;
			par_i++;
	...
	so->curr_partitions = par_i;
	so->total_partitions += par_i;
	so->curr_level = subtreerefdes->level + 1;
	lpath->alp_subtreerefdess[index - 1].in_use = false;
	return true;
}

void
_abt_build_lpath_batch(IndexScanDesc scan)
{
	ABTScanOpaque	so = (ABTScanOpaque) scan->opaque;
	...
	/* optimized path for fast sampling in small ranges */
	
	if (so->locked_path == NULL)
	{
		_abt_build_lpath_for_fast_sampling_batch(scan);
	}

	_abt_precompute_leaf_partition(scan);
}

void
_abt_build_lpath_for_fast_sampling(IndexScanDesc scan)
{
    Relation            rel = scan->indexRelation;
    ABTScanOpaque       so = (ABTScanOpaque) scan->opaque;
    ABTCachedAggInfo    agg_info = so->agg_info;
...
slow_retry:
	...
	/* 
     * We descend in the tree from the root whenever the page we're at is an
     * internal page with only one sub-tree whose key range matches the search
     * key range. When we break from the loop, we will either end up on a leaf
     * page, or the highest internal page with at least two matching sub-trees.
     *
     * TODO describe the loop invariant of page, opaque, buf, minoff
     * and maxoff.
	 */
    for (;;)
    {
        ...
        {
            /* 
             * If we're on a leaf page & we know we have at least one matching
             * record, we have located and locked the LR path (which one has
             * one page).
             */
            break;
        }
        ...
        /* 
         * Internal page: if this only has one matching sub-tree, descend into
         * it and continue the loop.
         */
        if (minoff == maxoff)
        {
            bool has_concurrent_smo;

            itemid = PageGetItemId(page, minoff);
            itup = (IndexTuple) PageGetItem(page, itemid);

            /* the sub-tree root to descend into*/
            cblkno = ABTreeTupleGetDownLink(itup); 

            /* 
             * save a copy of the expected high key to detect concurrent splits
             */
            if (minoff == PageGetMaxOffsetNumber(page))
            {
                if (ABT_P_RIGHTMOST(opaque))
            ...
            /*
             * Now we descend into the unique sub-tree that matches our search
             * range.
             *
             * However, here's a rare but possible race condition: the sub-tree
             * root is concurrently split, and then we need to backtrack to at
             * least the previous node (in most cases), or, in extremely rare
             * cases where several concurrent splits happened to be squeezed in
             * before us, further up.
            */
            buf = _abt_relandgetbuf(rel, buf, cblkno, ABT_READ); 
            buf = _abt_moveright_to_next_live_page(rel, buf);
            ...
            {
                /* should not happen */
                elog(ERROR, "we should not end up on a rightmost half-dead page");
            }
            ...
            {
                /* 
                 * If this is already the rightmost, there's no concurrent
                 * split.
                 */
                has_concurrent_smo = false;
            }
            else
            {
                int cmpres;
                cmpres = _abt_compare(rel, inskey_hikey, page, ABTP_HIKEY);
                Assert(cmpres >= 0);
                has_concurrent_smo = cmpres > 0;
            }

            if (has_concurrent_smo)
            {
                /* 
                 * Oops. We need to back-track. It may not worth the effort to
                 * really back-track. Let's do the search again from root and
                 * hope that concurrent splits are rare (they should be
                 * typically).
                 *
                 */
                _abt_relbuf(rel, buf);
            ...
            /* 
             * We're now on the correct unique sub-tree root. Continue the
             * quick descent loop.
             *
             * MUST continue the for(;;) loop here.
             */
        }
        else /* if (minoff < maxoff) */
        {
            ...
            /* 
             * Otherwise, we have found the lowest internal page with at least
             * two matching sub-trees. 
             */
            break;
        }

        /* end of the quick descent loop */
    }
    
    /*
     * To create the locked (lmost & rmost) path, we need to first identify and
     * lock the left-most and right-most leaf pages that intersects with the
     * search key range, without holding any locks above them from top down.
     *
     * Then we lock the pages on the leftmost and rightmost paths from bottom
     * up.  If we find any concurrent split at the sub-tree root `buf`
     * currently points to w, we must retry from the beginning, as we need to
     * grow the size of the lpath that we have already created at this point.
     */
    lpath = _abt_create_lpath(buf, minoff, maxoff);

    /* 
     * Top-down search. We maintain the following invariants:
     *
     * lbuf: locked and pinned page at the `level` on the left-most path
     * rbuf: locked and pinned page at the `level` on the right-most path
     * minoff: the first offset that intersects the search range (on lbuf)
     * maxoff: the last offset that intersects the search range (on rbuf)
     * plevel: the parent pages' level
     */
    lbuf = rbuf = buf;
    for (plevel = lpath->alp_level; plevel > 0; --plevel)
    {
        BlockNumber lcblkno;
        BlockNumber rcblkno;
        ...
        itemid  = PageGetItemId(page, minoff);
        itup = (IndexTuple) PageGetItem(page, itemid);
        lcblkno = ABTreeTupleGetDownLink(itup);
        ...
        itup = (IndexTuple) PageGetItem(page, itemid);
        rcblkno = ABTreeTupleGetDownLink(itup);
        ...
        /* prefetch on the right-most path */
        Assert(lcblkno != rcblkno);
        (void) PrefetchBuffer(rel, MAIN_FORKNUM, rcblkno);
    
        /* fetch and locate the page at plevel - 1 on the left-most path */
        lbuf = _abt_getbuf(rel, lcblkno, ABT_READ);
        if (so->inskey_low)
        {
            lbuf = _abt_moveright(rel, so->inskey_low, lbuf,
                                  /*forupdate =*/false, /*stack=*/NULL,
                                  ABT_READ, scan->xs_snapshot,
                                  agg_info);
            if (lcblkno != BufferGetBlockNumber(lbuf))
            {
                lcblkno = BufferGetBlockNumber(lbuf);
            }
            minoff = _abt_binsrch(rel, so->inskey_low, lbuf);
        }
        else
        {
            page = BufferGetPage(lbuf);
        ...
        /* fetch and locate the page at plevel - 1 on the right-most path */
        rbuf = _abt_getbuf(rel, rcblkno, ABT_READ);
        if (so->inskey_high)
        {
            rbuf = _abt_moveright(rel, so->inskey_high, rbuf,
                                  /*forupdate =*/false, /*stack=*/NULL,
                                  ABT_READ, scan->xs_snapshot,
                                  agg_info);
            if (rcblkno != BufferGetBlockNumber(rbuf))
            {
                rcblkno = BufferGetBlockNumber(rbuf);
            }
            maxoff = _abt_binsrch(rel, so->inskey_high, rbuf);
            if (plevel == 1)
            {
                /* goback is always true for the high key */
                maxoff = OffsetNumberPrev(maxoff);
        ...
        lpath->alp_lpd[2 * plevel - 1].alp_blkno = lcblkno;
        lpath->alp_lpd[2 * plevel - 1].alp_off = minoff;
        lpath->alp_lpd[2 * plevel - 1].alp_buf = InvalidBuffer;
        lpath->alp_lpd[2 * plevel - 2].alp_blkno = rcblkno;
        lpath->alp_lpd[2 * plevel - 2].alp_off = maxoff;
        lpath->alp_lpd[2 * plevel - 2].alp_buf = InvalidBuffer;
    }


    /* 
     * Bottom-up loop for locking the path. We also pre-compute and cache the
     * agg weights along the path ahead of time. Note that the leaf pages have
     * been locked.
     *
     * Note we are going to hold the lwlocks for a long time, so resume
     * the interrupts here.
     */
    nsubtrees = 0;

    RESUME_INTERRUPTS();
    /* leaf data entries are counted as separate sub-trees for convenience */
    if (lbuf == rbuf)
    {
        nsubtrees += maxoff - minoff + 1;
    }
    else
    {
        page = BufferGetPage(lbuf);
        nsubtrees += PageGetMaxOffsetNumber(page) - minoff + 1;
        lpath->alp_lpd[1].alp_buf = lbuf;

        page = BufferGetPage(rbuf);
        opaque = (ABTPageOpaque) PageGetSpecialPointer(page);
        nsubtrees += maxoff - ABT_P_FIRSTDATAKEY(opaque) + 1;
        lpath->alp_lpd[0].alp_buf = rbuf;

        RESUME_INTERRUPTS();
    }
    
    for (plevel = 1; plevel < lpath->alp_level; ++plevel)
    {
        OffsetNumber off;
        BlockNumber blkno;

        /* left */
        blkno = lpath->alp_lpd[plevel * 2 + 1].alp_blkno;
        lbuf = _abt_getbuf(rel, blkno, ABT_READ);
        RESUME_INTERRUPTS();
        
        off = _abt_find_downlink(rel, &lbuf,
                                 lpath->alp_lpd[plevel * 2 + 1].alp_off,
                                 lpath->alp_lpd[plevel * 2 - 1].alp_blkno,
                                 /*can_moveright =*/true);
        page = BufferGetPage(lbuf);
        blkno = BufferGetBlockNumber(lbuf);
        lpath->alp_lpd[plevel * 2 + 1].alp_blkno = blkno;
        lpath->alp_lpd[plevel * 2 + 1].alp_off= off;
        lpath->alp_lpd[plevel * 2 + 1].alp_buf = lbuf;

        /* The subtree at off only partially match, so it doesn't count. */
        nsubtrees += PageGetMaxOffsetNumber(page) - off; 

        /* right */
        blkno = lpath->alp_lpd[plevel * 2].alp_blkno;
        rbuf = _abt_getbuf(rel, blkno, ABT_READ);
        RESUME_INTERRUPTS();

        off = _abt_find_downlink(rel, &rbuf,
                                 lpath->alp_lpd[plevel * 2].alp_off,
                                 lpath->alp_lpd[plevel * 2 - 2].alp_blkno,
                                 /*can_moveright =*/true);
        page = BufferGetPage(rbuf);
        blkno = BufferGetBlockNumber(rbuf);
        lpath->alp_lpd[plevel * 2].alp_blkno = blkno;
        lpath->alp_lpd[plevel * 2].alp_off = off;
        lpath->alp_lpd[plevel * 2].alp_buf = rbuf;

        opaque = (ABTPageOpaque) PageGetSpecialPointer(page);
        nsubtrees += off - ABT_P_FIRSTDATAKEY(opaque);
    }
    ...
    if (lbuf != rbuf)
    {
        ...
        /* We have on last page to lock -- the subtree root. This has to be
         * specially handled because, if this page was split concurrently,
         * we must retry from the top. */
        lpath->alp_root_buf = _abt_getbuf(rel, lpath->alp_root_blkno, ABT_READ);
        RESUME_INTERRUPTS();
        page = BufferGetPage(lpath->alp_root_buf);

        /* 
         * In most of the cases, we should be able to find the loff and roff
         * immediately.
         */
        off = _abt_find_downlink(rel, &lpath->alp_root_buf,
                                 lpath->alp_loff,
                                 lpath->alp_lpd[plevel * 2 - 1].alp_blkno,
                                 /*can_moveright =*/false);
        if (off == InvalidOffsetNumber)
        {
            _abt_release_lpath_for_fast_sampling(rel, lpath);
            goto slow_retry;
        }
        lpath->alp_loff = off;
        ...
        /* anything between loff and roff is a completely covered subtree */
        Assert(lpath->alp_loff < lpath->alp_roff);
        nsubtrees += (lpath->alp_roff - lpath->alp_loff - 1);
    }
    
    /* 
     * For convenience, we also duplicate the buffer id of the root page in the
     * last slot of alp_lpd array, so that one can always access the page
     * using the idx stored in an ABTLockedPathSubtreeRef.
     */
    lpath->alp_lpd[2 * lpath->alp_level].alp_buf = lpath->alp_root_buf;

    /* allocate alias and tid arrays */
    if (nsubtrees > PG_UINT16_MAX)
    {
        elog(ERROR, "There are too many subtrees (%lu) in the locked AB-tree "
            "path. Plesae consider disabling the locked path optimization "
            "(SET abtree_lock_lrpath_during_sampling TO OFF).", nsubtrees);
    }
    lpath->alp_nsubtrees = 0;
    lpath->alp_total_aggs = 0;
    aggs = (uint64 *) palloc(sizeof(uint64) * nsubtrees);
    lpath->alp_subtreerefs = (ABTLockedPathSubtreeRef *)
        palloc(sizeof(ABTLockedPathSubtreeRef) * nsubtrees);
   
    for (i = 0; i <= lpath->alp_level * 2; ++i)
    {
        ABTLockedPathSubtreeRef subtreeref;
        bool is_leaf = i < 2;
        bool is_root = i == lpath->alp_level * 2;
        ...
        if (is_root)
        {
            ...
            if (is_leaf)
            {
                minoff = lpath->alp_loff;
                maxoff = lpath->alp_roff;
            }
            else
            {
                minoff = lpath->alp_loff + 1;
                maxoff = lpath->alp_roff - 1;
            }
        }
        else
        {
            page = BufferGetPage(lpath->alp_lpd[i].alp_buf);
            opaque = (ABTPageOpaque) PageGetSpecialPointer(page);
            if (i & 1)
            {
                /* left */
                if (is_leaf)
                {
                    minoff = lpath->alp_lpd[i].alp_off;
                }
                else
                {
                    minoff = lpath->alp_lpd[i].alp_off + 1;
                }
                maxoff = PageGetMaxOffsetNumber(page);
            }
            else
            {
                ...
                if (is_leaf)
                {
                    maxoff = lpath->alp_lpd[i].alp_off;
                }
                else
                {
                    maxoff = lpath->alp_lpd[i].alp_off - 1;
                }
            }
        }
        ...
        if (is_leaf)
        {
            ...
            for (off = minoff; off <= maxoff; ++off)
            {
                ...
                else
                {
                    ...
                    xid = (TransactionId) pg_atomic_read_u32(
                         (pg_atomic_uint32 *) xid_ptr);
                    if (_abt_xid_in_snapshot(xid, so->snapshot))
                        continue; 
                ...
                aggs[lpath->alp_nsubtrees] = agg;
                subtreeref.idx = i;
                subtreeref.off = off;
                lpath->alp_subtreerefs[lpath->alp_nsubtrees] = subtreeref;
                ++lpath->alp_nsubtrees;
            }
        }
        else
        {
            ...
            for (off = minoff; off <= maxoff; ++off)
            {
                ...
                agg = _abt_read_agg(rel, itup, so->snapshot, agg_info);
                if (agg == 0)
                    continue;
    ...
    /* 
     * Precompute the proportion of the weight of each subtree in the total
     * weights. This will be needed in _abt_fast_sampling.
     *
     * Must be done prior to building the alias structure as
     * aqp_alias_u64_create will overwrite and repurpose the aggs array.
     */
    lpath->alp_subtree_aggs = (uint64 *)
        palloc(sizeof(uint64) * lpath->alp_nsubtrees);
    memcpy(lpath->alp_subtree_aggs, aggs,
           sizeof(uint64) * lpath->alp_nsubtrees);

    /* build alias structure */
    lpath->alp_subtree_alias = aqp_alias_u64_create(aggs, lpath->alp_nsubtrees);
    lpath->alp_total_aggs =
        aqp_alias_u64_get_sum_weights(lpath->alp_subtree_alias);

    /* initialize random state */
    sampler_random_init_state(random(), lpath->alp_randstate);

    /* save and we're done with constructing the locked path */
    so->locked_path = lpath; 
}

void
_abt_build_lpath_for_fast_sampling_batch(IndexScanDesc scan)
{
    Relation            rel = scan->indexRelation;
    ABTScanOpaque       so = (ABTScanOpaque) scan->opaque;
    ABTCachedAggInfo    agg_info = so->agg_info;
...
slow_retry:
	...
	lpath->alp_subtreerefdess = (ABTLockedPathSubtreeRefDes *)
        palloc(sizeof(ABTLockedPathSubtreeRefDes) * 10000);
    ...
    for (i = 0; i <= lpath->alp_level * 2; ++i)
    {
        ...
        if (is_leaf)
        {
			...
			lpath->alp_nleaf = lpath->alp_nsubtrees;
        }
        else
        {
            OffsetNumber off;
            ...
            for (off = minoff; off <= maxoff; ++off)
            {
				...
				*/
				...
				if (i == lpath->alp_level * 2)
				{
					subtreerefdes.buf = lpath->alp_lpd[i].alp_buf;
					subtreerefdes.blkno = lpath->alp_lpd[i].alp_blkno;
					subtreerefdes.minoff = off;
					subtreerefdes.maxoff = off;
					subtreerefdes.level = lpath->alp_level;
					subtreerefdes.agg = agg;
					subtreerefdes.is_leaf = false;
					subtreerefdes.in_use = true;
					subtreerefdes.is_splited = false;
					lpath->alp_subtreerefdess[lpath->alp_nrangesubtrees] 
						= subtreerefdes;
				

                    /* elog(INFO, "index: %d, key: %d", lpath->alp_nrangesubtrees,
                        DatumGetInt32(index_getattr(itup, so->keyData->sk_attno, itupdesc, &isNull))); */
					++lpath->alp_nrangesubtrees;
				}
			...
			if (i != lpath->alp_level * 2 && minoff <= maxoff)
			{
                subtreerefdes.buf = InvalidBuffer;
				subtreerefdes.blkno = lpath->alp_lpd[i].alp_blkno;
				subtreerefdes.minoff = minoff;
				subtreerefdes.maxoff = maxoff;
				subtreerefdes.level = lpath->alp_level / 2;
				subtreerefdes.is_leaf = false;
				subtreerefdes.in_use = true;
				subtreerefdes.is_splited = false;
				lpath->alp_subtreerefdess[lpath->alp_nrangesubtrees] 
		...
		so->total_partitions = lpath->alp_nrangesubtrees + 1;

		/*
		lpath->alp_subtreerefdess[i].buf = lpath->alp_lpd[i].alp_buf;
		lpath->alp_subtreerefdess[i].blkno = lpath->alp_lpd[i].blkno;
		lpath->alp_subtreerefdess[i].minoff = minoff;
		lpath->alp_subtreerefdess[i].maxoff = maxoff;
		lpath->alp_subtreerefdess[i].agg = tot_par_agg;
		lpath->alp_subtreerefdess[i].in_use = true;

		if (is_leaf)
			lpath->alp_subtreerefdess[i].is_leaf = true;
		else
			lpath->alp_subtreerefdess[i].is_leaf = false; 
		*/
    }
	...
	/*
	for (i = 0; i <= lpath->alp_level * 2; ++i)
		lpath->alp_subtreerefdess[i].weight = lpath->alp_subtreerefDess[i].agg /
											lpath->alp_total_aggs;
	*/
...
bool
_abt_fast_sampling_batch(IndexScanDesc scan, double r)
{
    Relation rel = scan->indexRelation;
    ABTScanOpaque so = (ABTScanOpaque) scan->opaque;
    ABTLockedPath lpath = so->locked_path;
    ...
    uint32 subtree_idx;
    ABTLockedPathSubtreeRefDes *subtreerefdes;
    ABTFastSamplingContextData fsctx;
    bool ret;
    
    if (lpath->alp_root_blkno == InvalidBlockNumber)
    {
        so->inv_prob = get_float8_infinity();
        return false;
    }
    
    x = (uint64) lpath->alp_total_aggs * r;
    if (x == lpath->alp_total_aggs)
    {
        --x; 
    }

    subtree_idx = so->idx - 1;
    subtreerefdes = &lpath->alp_subtreerefdess[subtree_idx];
    
    /*
     * Set up rootbuf and the modified random number r to pass to
     * _abt_sample_extended, such that rootbuf contains an s-locked buffer
     * that points to the subtree root we are sampling/returning record from.
     */

	if (subtreerefdes->blkno != 0)
	{
		fsctx.rootbuf = _abt_getbuf(rel, subtreerefdes->blkno, ABT_READ);
		fsctx.leafoff = 0;
		fsctx.expected_hikey = NULL;
		fsctx.minoff = subtreerefdes->minoff;
		fsctx.maxoff = subtreerefdes->maxoff;
	}
	else
    {
			/*
			* This is a subtree root page that we need to lock by ourselves. We
			* also need to grab the expected_hikey from its already locked parent
			* page so that we can deal with concurrent SMO.
			*/
			Page ppage;
			ABTPageOpaque popaque;
			ItemId itemid;
			IndexTuple itup;
			BlockNumber cblkno;
			...
			itemid = PageGetItemId(ppage, subtreerefdes->minoff);
			itup = (IndexTuple) PageGetItem(ppage, itemid);
			cblkno = ABTreeTupleGetDownLink(itup);

			if (subtreerefdes->minoff == PageGetMaxOffsetNumber(ppage))
			{
				if (ABT_P_RIGHTMOST(popaque))
				{
					itup = NULL;
				}
				else
				{
					itemid = PageGetItemId(ppage, ABTP_HIKEY);
					itup = (IndexTuple) PageGetItem(ppage, itemid);
				}
			}
			else
			{
				itemid = PageGetItemId(ppage, subtreerefdes->minoff + 1);
				itup = (IndexTuple) PageGetItem(ppage, itemid);
			}
	
			/*
			* XXX(6/5/24) This could lead to deadlock when a concurrent SMO
			* performs lock coupling from bottom up since the parent page is
			* already locked. Also see _abt_getstackbuf() in abtinsert.c.
			*/
			fsctx.rootbuf = _abt_getbuf(rel, cblkno, ABT_READ);
			fsctx.leafoff = 0;
			/* page will be kept locked -- no need to copy */
			fsctx.expected_hikey = itup; 
			
			/*
			* TODO If we really want repeatability, we need to rethink how
			* closely we need to track the insertion and deletions in the tree.
			*/
			fsctx.sampled_item_weight = subtreerefdes->agg;
			r = sampler_random_fract(lpath->alp_randstate);

			so->split_partition = false;
    }

	r = sampler_random_fract(lpath->alp_randstate);
    
    while (!(ret = _abt_sample_extended(scan, r, &fsctx)) &&
           isinf(so->inv_prob))
    {
        CHECK_FOR_INTERRUPTS();
        /* Assert(subtreeref->idx >= 2); */
        r = sampler_random_fract(lpath->alp_randstate);
    }
    so->inv_prob = so->inv_prob; /* * (float8) 
					(lpath->alp_total_aggs - so->statis[0].sample_size) /
                    lpath->alp_subtree_aggs[subtree_idx]; */
...
bool
_abt_fast_sampling(IndexScanDesc scan, double r)
{
    Relation rel = scan->indexRelation;
    ABTScanOpaque so = (ABTScanOpaque) scan->opaque;
    ABTLockedPath lpath = so->locked_path;
    ...
    subtree_idx = aqp_alias_u64_sample_unif_u64(lpath->alp_subtree_alias, x);
    Assert(subtree_idx >= 0 && subtree_idx < lpath->alp_nsubtrees);
    subtreeref = &lpath->alp_subtreerefs[subtree_idx];
     ...
     * that points to the subtree root we are sampling/returning record from.
     */
	/* necessary for pswr with tree height */
	so->sample_height = subtreeref->idx / 2;

    if (subtreeref->idx < 2)
    {
        /*
         * This is a (ref to a) data entry in the leaf node.
         */
        fsctx.rootbuf = lpath->alp_lpd[subtreeref->idx].alp_buf;
        fsctx.leafoff = subtreeref->off;
        fsctx.expected_hikey = NULL; /* not needed */
        
        /* 
         * We pass in the posting tuple's aggregate weight here so that
         * we don't have to re-compute it. _abt_sample_extended should be able
         * to do one linear scan to find the correct posting offset.
         *
         * For regular leaf tuple, this should be exactly the same as its
         * stored weight (if any), or 1 for uniform sampling.
         */
        fsctx.sampled_item_weight = lpath->alp_subtree_aggs[subtree_idx];
    }
    else
    {
        /*
        ...
        ppage = BufferGetPage(lpath->alp_lpd[subtreeref->idx].alp_buf);
        popaque = (ABTPageOpaque) PageGetSpecialPointer(ppage);
        itemid = PageGetItemId(ppage, subtreeref->off);
        itup = (IndexTuple) PageGetItem(ppage, itemid);
        cblkno = ABTreeTupleGetDownLink(itup);

        if (subtreeref->off == PageGetMaxOffsetNumber(ppage))
        {
            if (ABT_P_RIGHTMOST(popaque))
            {
                itup = NULL;
        ...
        {
            itemid = PageGetItemId(ppage, subtreeref->off + 1);
            itup = (IndexTuple) PageGetItem(ppage, itemid);
        }
    ...
    so->inv_prob = so->inv_prob * (float8) lpath->alp_total_aggs /
                    lpath->alp_subtree_aggs[subtree_idx];
    return ret;
}

void
_abt_release_lpath_for_fast_sampling(Relation rel, ABTLockedPath lpath)
{
    uint32 i;
    
    if (lpath->alp_root_blkno != InvalidBlockNumber)
    {
        if (lpath->alp_root_buf != InvalidBuffer)
        {
            HOLD_INTERRUPTS();
            _abt_relbuf(rel, lpath->alp_root_buf);
        }
    }

    for (i = 0; i < lpath->alp_level * 2; ++i)
    {
        if (lpath->alp_lpd[i].alp_buf != InvalidBuffer)
        {
            HOLD_INTERRUPTS();
            _abt_relbuf(rel, lpath->alp_lpd[i].alp_buf);
        }
    }	

/*
	if (lpath->alp_subtreerefdess)
	{
		for (i = lpath->alp_nsubtrees - lpath->alp_nleaf; i < 10000; ++i)
		{
			if (lpath->alp_subtreerefdess[i].buf != InvalidBuffer)
			{
				HOLD_INTERRUPTS();
				_abt_relbuf(rel, lpath->alp_subtreerefdess[i].buf);
			}
		}
	}
*/
    
    if (lpath->alp_subtree_aggs)
        pfree(lpath->alp_subtree_aggs);
    if (lpath->alp_subtree_alias)
        aqp_alias_u64_destroy(lpath->alp_subtree_alias);
    if (lpath->alp_subtreerefs)
        pfree(lpath->alp_subtreerefs);
    if (lpath->alp_subtreerefdess)
        pfree(lpath->alp_subtreerefdess);
    pfree(lpath);
}

static void
_abt_set_dummy_lpath(ABTScanOpaque so)
{
    so->locked_path =
        (ABTLockedPath) palloc0(offsetof(ABTLockedPathData, alp_lpd));
    so->locked_path->alp_root_blkno = InvalidBlockNumber;
}

static ABTLockedPath
_abt_create_lpath(Buffer buf, OffsetNumber loff, OffsetNumber roff)
{
    ABTLockedPath lpath;
    Page root = BufferGetPage(buf);
    ABTPageOpaque opaque = (ABTPageOpaque) PageGetSpecialPointer(root);
    uint32 level = opaque->abtpo.level;

    lpath = (ABTLockedPath) palloc0(offsetof(ABTLockedPathData, alp_lpd) +
        sizeof(ABTLockedPageData) * (level * 2 + 1));
    lpath->alp_root_blkno = BufferGetBlockNumber(buf);
    lpath->alp_level = level;
    lpath->alp_root_buf = buf;
    lpath->alp_loff = loff;
    lpath->alp_roff = roff;
...
static OffsetNumber
_abt_find_downlink(Relation rel,
                   Buffer *p_buf,
                   OffsetNumber start_off,
                   BlockNumber cblkno,
                   bool can_moveright)
{
    ...
    off = _abt_try_find_downlink(page, start_off, cblkno);
    if (off != InvalidOffsetNumber)
    {
        return off;
    }
    ...
    for (;;)
    {
        ...
        *p_buf = _abt_getbuf(rel, blkno, ABT_READ);
        page = BufferGetPage(*p_buf);
    }
