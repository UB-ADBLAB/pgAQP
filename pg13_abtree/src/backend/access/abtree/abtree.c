...
/*
 * Whether to bypass the version store search when we read an aggregate weight
 * for a specific sub-tree. This is disabled by default, which is the original
 * behavior in in the VLDB'22 AB-tree implementation.
 *
 * Enabling the bypass may reduce the contention in the hash table lookup and
 * in turn improve the sampling throughput, at the cost of heavy deletion in
 * the background may hike the rejection rate of a sampling process at an old
 * snapshot.
 *
 * See _abt_read_agg() in abtversionstore.c.
 */
bool abtree_read_agg_bypass_version_store = false;

/*
 * Optimization for sampling in an AB-tree over a very small key range that
 * happens to span exactly two adjacent sub-trees, which could result in
 * arbitrarily large rejection rate hikes without locking.
 *
 * See TODO ??? for rationale.
 */
bool abtree_lock_lrpath_during_sampling = false;
...
abthandler(PG_FUNCTION_ARGS)
{
	...
	amroutine->amsamplebatchtuple = abtsamplebatchtuple;
...
abtsampletuple(IndexScanDesc scan, double random_number)
{
    ...
    if (!abtree_lock_lrpath_during_sampling)
    {
        /* 
        ...
        if (scan->kill_prior_tuple)
        {
            ...
            _abt_killitems(scan);
            /* 
             * Reset the killedItems because it points to some temporary
             * location
             */
            so->killedItems = NULL;
        }
    ...
    else
    {
        ...
        {
            _abt_build_lpath_for_fast_sampling(scan);
        }
        ...
        res = _abt_fast_sampling(scan, random_number);
    }
...
/*
 * abtsamplebatchtuple() -- Sample the next batch of tuples.
 */
bool
 abtsamplebatchtuple(IndexScanDesc scan, double r)
 {
	ABTScanOpaque	so = (ABTScanOpaque) scan->opaque;
	...
	res = _abt_fast_sampling_batch(scan, r);
	}
	else
	res = _abt_gettuple_from_leaf(scan);
...
abtbeginscan(Relation rel, int nkeys, int norderbys)
{
    ...
    so->idx = 0;
    so->idx_leaf = 0;
    so->total_partitions = 0;
    so->curr_partitions = 0;
    so->curr_level = 0;
    so->initial_sample_size = 0;
    so->sample_height = 0;	
...
abtrescan(IndexScanDesc scan, ScanKey scankey, int nscankeys,
    ...
    if (so->locked_path)
    {
        _abt_release_lpath_for_fast_sampling(scan->indexRelation,
                                             so->locked_path);
        so->locked_path = NULL;
    }
