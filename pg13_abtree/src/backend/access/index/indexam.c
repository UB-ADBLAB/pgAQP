...
/* ----------------
 * index_getnext_tid - sample the next TID from a scan
 *
 * random_number is a random number in range (0, 1), which acts as the source
 * of randomness.
 *
 * The result is the next sampled TID satisfying the scan keys, or NULL if the
 * sample was rejected and we need to retry.
 * ----------------
 */
ItemPointer
index_samplenext_tid(IndexScanDesc scan, double random_number)
{
	bool		found;

	SCAN_CHECKS;
	CHECK_SCAN_PROCEDURE(amsampletuple);

	Assert(TransactionIdIsValid(RecentGlobalXmin));

	/*
	 * The AM's amsampletuple proc samples a random entry matching the scan
	 * keys, and puts the TID into scan->xs_heaptid.  It should also set
	 * scan->xs_recheck and possibly scan->xs_itup/scan->xs_hitup, though we
	 * pay no attention to those fields here.
	 *
	 * If found is false, it does not mean there's no tuple in the range.
	 * Rather, that indicates the sample was rejected due to internal reasons
	 * and the caller should retry.
	 */
	found = scan->indexRelation->rd_indam->amsampletuple(scan, random_number);
...
/* ----------------
 * index_samplenextbatch_tid - sample the next batch of TIDs from a scan
 *
 * random_number is a random number in range (0, 1), which acts as the source
 * of randomness.
 *
 ...
 */
ItemPointer
index_samplenextbatch_tid(IndexScanDesc scan, double r)
{
	bool		found;

	SCAN_CHECKS;
	CHECK_SCAN_PROCEDURE(amsamplebatchtuple);
	 ...
	 * and the caller should retry.
	 */
	/* found = scan->indexRelation->rd_indam->amsamplebatchtuple(scan, sample_size); */
    found =	scan->indexRelation->rd_indam->amsamplebatchtuple(scan, r);
...
bool
index_samplenext_slot(IndexScanDesc scan, double random_number,
					  TupleTableSlot *slot)
{
	ItemPointer tid;

	/* 
	 * We should have already asserted that this is an MVCC snapshot in the
	 * caller, so there can't be multiple valid tuples for one sampled TID. 
	 */
	Assert(!scan->xs_heap_continue);

	/* Time to fetch the next TID from the index */
	tid = index_samplenext_tid(scan, random_number);

	/* Sample rejected. Let the caller retry or check interrupt. */
	if (tid == NULL)
		return false;
	Assert(ItemPointerEquals(tid, &scan->xs_heaptid));
	...
	/* 
	 * Not a visible tuple, it's treated as a rejected sample. 
	 */
	return false;
}
