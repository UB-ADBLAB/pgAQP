...
aqp_tablesample_swr_tsm_handler(PG_FUNCTION_ARGS)
{
    ...
    /* TODO maybe allow locking the index to make it repeatable? */
...
PG_FUNCTION_INFO_V1(aqp_tablesample_pswr_tsm_handler);
Datum
aqp_tablesample_pswr_tsm_handler(PG_FUNCTION_ARGS)
{
    TsmRoutine *tsm = makeNode(TsmRoutine);
     ...
     * 3: Target absolute error epsilon \varepsilon
     * 4: Target confidence 1 - \delta
     */
    tsm->parameterTypes =
        list_make4_oid(INT8OID, INT8OID, FLOAT8OID, FLOAT8OID);
    /* TODO maybe allow locking the index to make it repeatable? */
    tsm->repeatable_across_queries = false;
    /* 
     * This is a fake value to force set_tablesample_rel_pathlist() not to
...
aqp_tablesample_swr_begin_sample_scan(SampleScanState *node,
                                      ...
                                      uint32 seed)
{
    elog(ERROR, "tablesample swr()/pswr() should be rewritten as a "
